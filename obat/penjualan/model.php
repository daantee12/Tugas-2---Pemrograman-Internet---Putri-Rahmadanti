...
public function insert($pembeli, $kode_obat, $jumlah)
{
    $total_belanja = 0;
    $aa = "SELECT * FROM data_obat WHERE kode_obat='$kode_obat'";
    ...
    if (!empty($baris)) 
    {
        ...
        $barang = $baris[0]->nama;
        $dda = "UPDATE data_obat SET  stok='$stok_baru' WHERE kode_obat='$kode_obat'";
        $this->conn->query($dda);
    }else{
        echo "<center>
    ...
    '$kode_obat','$barang', '$jumlah', '$total_belanja')";
    $this->conn->query($sql);
}
...
$bind = $this->conn->query($sql); 
    ...
    public function update($kode_penjualan, $pembeli, $kode_barang, $jumlah)
    {
        ...
        $total_belanja =  $baris[0]->harga*$jumlah;
        // var_dump($baris);
        $dda = "UPDATE data_obat SET stok='$jumlah_baru' WHERE kode_obat='$kode_obat'";
        $this->conn->query($dda);
